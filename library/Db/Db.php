...
abstract class Db {
	...
	protected static $_is_replace_sql = true;
	...
	/**
	 * 获取Db实例，可选择主从
	 * @param bool $is_replace_sql
	 * @param bool $master
	 * @return mixed
	 */
	public static function getInstance($is_replace_sql = true, $master = false) {
		if (self::$_servers == null) {
			self::$_servers = Yaf\Registry::get('database');
		}

		self::$_is_replace_sql = $is_replace_sql;
	...
	public function query($sql) {
        ...
        if ($this->getNumberError()) {
            Log::sqlError('SQL', $this->getMsgError() . ':' . $sql);
        }else{
            Log::sql("SQL", $sql);
        }
		return $this->result;
	 ...
	 * @return bool
	 */
	public function insert($table, $data, $null_values = false){
        if (!$data && !$null_values)
            return true;
...
//        }
		...
		foreach ($data as $row_data) {
			...
			foreach ($row_data as $key => $value) {
				...
				$values[] = ":{$key}{$n}";
				if ($value['type'] == 'sql') {
					$param[":{$key}{$n}"] = $value['value'];
                }else {
					$param[":{$key}{$n}"] = $null_values && ($value['value'] === '' || is_null($value['value'])) ? NULL : $value['value'];
                }
            }
			$n++;
		...
		return (bool)$this->exec($sql, $param);
	}
    ...
    public function parseWhere($option, $prefix=''){
        ...
        if(!empty($prefix) && is_string($prefix)){
            $prefix_field = $prefix.'.';
            $prefix_bind = $prefix.'_';
        }else{
            $prefix_field = $prefix_bind = '';
        }
        foreach ($option as $k=>$v){
            switch ($k){
                case 'like':
                    foreach ($v as $kk=>$vv){
                        $where[] = " {$prefix_field}{$kk} LIKE :{$prefix_bind}{$kk}";
                        $param[":{$prefix_bind}{$kk}"] = $vv;
                    }
                    break;
                case 'binary':
                    foreach ($v as $kk=>$vv){
                        $where[] = "binary {$prefix_field}{$kk}=:{$prefix_bind}{$kk}";
                        $param[":{$prefix_bind}{$kk}"] = $vv;
                    }
                    break;
                default:
                    if(is_array($v)){
                        $where_in = array();
                        foreach ($v as $vk=>$vv){
                            $where_in[] = ":{$prefix_bind}{$k}_{$vk}";
                            $param[":{$prefix_bind}{$k}_{$vk}"] = $vv;
                        }
                        $where[] =  "{$prefix_field}{$k} IN (".implode(',',$where_in).")";
                    }else{
                        $where[] = "{$prefix_field}{$k}=:{$prefix_bind}{$k}";
                        $param[":{$prefix_bind}{$k}"] = $v;
                    }
                    break;
            }
        }
	 ...
	 * @return bool
	 */
	public function update($table, $data, $where = '', $limit = 0, $null_values = false) {
		if (!$data)
			return true;
		...
		foreach ($data as $key => $value) {
			...
			if ($value['type'] == 'sql'){
				...
				$param[":{$key}"] = ($null_values && ($value['value'] === '' || is_null($value['value']))) ? NULL : $value['value'];
			}
		}
		...
		if(!empty($where) && is_array($where)){
            ...
            foreach ($where as $k=>$v){
                if(is_array($v)){
                    $where_in = array();
                    foreach ($v as $vk=>$vv){
                        $where_in[] = ":where_{$k}_{$vk}";
                        $param[":where_{$k}_{$vk}"] = $vv;
                    }
                    $where_prepare[] =  "{$k} IN (".implode(',',$where_in).")";
                }else{
                    $where_prepare[] = "{$k}=:where_{$k}";
                    $param[":where_{$k}"] = $v;
                }
            }
			$where_prepare = implode(' AND ', $where_prepare);
...
//	}
     ...
     * @return mixed
     */
    public function getReplaceSql($sql){
        if(self::$_is_replace_sql == true){
            $sql = preg_replace('/{{(.*?)}}/', DB_TABLE_PREFIX.'\1', $sql);
        }
        return $sql;
    }

    public function exec($sql, $param=array()){
        return $this->getPrepareSth($sql, $param);
    }
    ...
    public function fetch($sql, $param=array()){
        $sth = $this->getPrepareSth($sql, $param);
     ...
     * @return mixed
     */
    protected function getPrepareSth($sql, $param=array()){
        $sql = $this->getReplaceSql($sql);
        $sth = $this->link->prepare($sql);
        $sth->execute($param);
        if ($sth->errorCode() == PDO::ERR_NONE) {
            Log::sql("SQL", str_replace(array_keys($param),array_values($param),$sql).' | p');
        }else{
            $err_msg = $sth->errorInfo();
            $err_msg = isset($err_msg[2]) ? $err_msg[2] : '';
            Log::sqlError('SQL', $err_msg . ':' . str_replace(array_keys($param),array_values($param),$sql).' | p');
        }
        return $sth;
    }
}
