...
class WxActivityGetnewSubscribeModel extends MBaseModel{
    protected $_table = '{{wx_activity_getnew_subscribe}}';
    ...
    public function setTableByPublicId($public_id){
        if(isset($public_id) && is_numeric($public_id)){
            $this->setTable('wx_activity_getnew_subscribe_'.$public_id);
        }
    }
    ...
    public function saveLink($wx_user_id, $subscribe_wx_user_id){
    ...
    public function getLinkNum($wx_user_id){
        $item = $this->MFind(array(
            'field' => 'COUNT(1) AS num',
            'where' => array('wx_user_id' => $wx_user_id,),
        ));
